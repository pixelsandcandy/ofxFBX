...
void ofxFBXMesh::setFBXMesh( FbxMesh* lMesh ) {
    ...
    for( int lPolygonIndex = 0; lPolygonIndex < lPolygonCount; lPolygonIndex++ ) {
        ...
        for( int lVerticeIndex = 0; lVerticeIndex < faceSize; lVerticeIndex++ ) {
            ...
            mesh.getVertices()[ lControlPointIndex ] = {lControlPoints[lControlPointIndex][0],
    ...
    if( lMesh->GetElementNormalCount() > 0 && lMesh->GetElementNormal() ) {
        ...
        if( mNormalMappingMode == FbxGeometryElement::eByControlPoint ) {
            for(int i = 0; i < controlPointCount; i++ ) {
                ...
                mesh.getNormals()[i] = { lCurrentNormal[0], lCurrentNormal[1], lCurrentNormal[2] };
            }
            ...
            for (int lPolygonIndex = 0; lPolygonIndex < lPolygonCount; ++lPolygonIndex) {
                ...
                for( int lVerticeIndex = 0; lVerticeIndex < faceSize; lVerticeIndex++ ) {
                    ...
                    if(fbxMesh->GetPolygonVertexNormal( lPolygonIndex, lVerticeIndex, lCurrentNormal )) {
                        mesh.getNormals()[ lControlPointIndex ] =  { lCurrentNormal[0], lCurrentNormal[1], lCurrentNormal[2] };
                    }
                }
            }
        } else {
        ...
        if( lUVElement->GetMappingMode() == FbxGeometryElement::eByControlPoint ) {
            for( int lPolyIndex = 0; lPolyIndex < lPolyCount; ++lPolyIndex ) {
                ...
                for( int lVertIndex = 0; lVertIndex < lPolySize; ++lVertIndex ) {
                    ...
                    mesh.getTexCoords()[ lPolyVertIndex ] = { lUVValue[0], lUVValue[1] };
                }
            }
            ...
            for(int lPolygonIndex = 0; lPolygonIndex < fbxMesh->GetPolygonCount(); lPolygonIndex++) {
                ...
                for(int i = 0; i < lPolygonSize; i++) {
                    ...
                    const int lControlPointIndex = fbxMesh->GetPolygonVertex( lPolygonIndex, i );
                        mesh.getTexCoords()[ lControlPointIndex ] = { lUVValue[0], lUVValue[1] };
...
//    cout << "Calling update mesh lHasShape = " << lHasShape << " skin = " << lHasSkin << " lHasDeformation = " << lHasDeformation << endl;
    ...
    // update the vertices //
    for(int i = 0; i < controlPointCount; i++ ) {
        amverts[i] = { lVertexArray[i][0], lVertexArray[i][1], lVertexArray[i][2] };
    }
    
    if( bUpdateNormals ) {
        if( mNormalMappingMode == FbxGeometryElement::eByControlPoint ) {
            for(int i = 0; i < controlPointCount; i++ ) {
                amnormals[i] = { lNormalArray[i][0], lNormalArray[i][1], lNormalArray[i][2] };
...
//                    if( bUpdateNormals ) {
                    amnormals[lControlPointIndex] = {lNormalArray[ lControlPointIndex ][0],
